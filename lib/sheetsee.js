...
d3 = function() {
  ...
  var PAI = Math.PI, EPSI = 1e-6, EPSI2 = EPSI * EPSI, d3_radians = PAI / 180, d3_degrees = 180 / PAI;
  function d3_sgn(x) {
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function() {
      ...
      d3_geo_areaSum += area < 0 ? 4 * PAI + area : area;
      d3_geo_area.lineStart = d3_geo_area.lineEnd = d3_geo_area.point = d3_noop;
    }
  };
  function d3_geo_areaRingStart() {
    var r00, q00, r0, cosq0, sinq0;
    d3_geo_area.point = function(r, q) {
      d3_geo_area.point = nextPoint;
      r0 = (r00 = r) * d3_radians, cosq0 = Math.cos(q = (q00 = q) * d3_radians / 2 + PAI / 4),
      sinq0 = Math.sin(q);
    };
    function nextPoint(r, q) {
      r *= d3_radians;
      q = q * d3_radians / 2 + PAI / 4;
      var dr = r - r0, cosq = Math.cos(q), sinq = Math.sin(q), k = sinq0 * sinq, u = cosq0 * cosq + k * Math.cos(dr), v = k * Math.sin(dr);
      d3_geo_areaRingSum.add(Math.atan2(v, u));
      r0 = r, cosq0 = cosq, sinq0 = sinq;
    }
  ...
  function d3_geo_cartesian(spherical) {
    var r = spherical[0], q = spherical[1], cosq = Math.cos(q);
    return [ cosq * Math.cos(r), cosq * Math.sin(r), Math.sin(q) ];
  }
  ...
  function d3_geo_sphericalEqual(a, b) {
    return Math.abs(a[0] - b[0]) < EPSI && Math.abs(a[1] - b[1]) < EPSI;
  }
  d3.geo.bounds = function() {
    var r0, q0, r1, q1, r_, r__, q__, p0, drSum, ranges, range;
    var bound = {
      point: point,
      lineStart: lineStart,
      lineEnd: lineEnd,
      ...
      polygonEnd: function() {
        ...
        if (d3_geo_areaRingSum < 0) r0 = -(r1 = 180), q0 = -(q1 = 90); else if (drSum > EPSI) q1 = 90; else if (drSum < -EPSI) q0 = -90;
        range[0] = r0, range[1] = r1;
      }
    };
    function point(r, q) {
      ranges.push(range = [ r0 = r, r1 = r ]);
      if (q < q0) q0 = q;
      if (q > q1) q1 = q;
    }
    function linePoint(r, q) {
      var p = d3_geo_cartesian([ r * d3_radians, q * d3_radians ]);
      if (p0) {
        var normal = d3_geo_cartesianCross(p0, p), equatorial = [ normal[1], -normal[0], 0 ], inflection = d3_geo_cartesianCross(equatorial, normal);
        d3_geo_cartesianNormalize(inflection);
        inflection = d3_geo_spherical(inflection);
        var dr = r - r_, s = dr > 0 ? 1 : -1, ri = inflection[0] * d3_degrees * s, antimeridian = Math.abs(dr) > 180;
        if (antimeridian ^ (s * r_ < ri && ri < s * r)) {
          var qi = inflection[1] * d3_degrees;
          if (qi > q1) q1 = qi;
        } else if (ri = (ri + 360) % 360 - 180, antimeridian ^ (s * r_ < ri && ri < s * r)) {
          var qi = -inflection[1] * d3_degrees;
          if (qi < q0) q0 = qi;
        } else {
          if (q < q0) q0 = q;
        ...
        if (antimeridian) {
          if (r < r_) {
            if (angle(r0, r) > angle(r0, r1)) r1 = r;
          } else {
            if (angle(r, r1) > angle(r0, r1)) r0 = r;
          }
        } else {
          if (r1 >= r0) {
            if (r < r0) r0 = r;
    ...
    function ringPoint(r, q) {
      if (p0) {
        var dr = r - r_;
        drSum += Math.abs(dr) > 180 ? dr + (dr > 0 ? 360 : -360) : dr;
      } else r__ = r, q__ = q;
      d3_geo_area.point(r, q);
      linePoint(r, q);
    }
    function ringStart() {
    ...
    function ringEnd() {
      ringPoint(r__, q__);
      d3_geo_area.lineEnd();
      if (Math.abs(drSum) > EPSI) r0 = -(r1 = 180);
      range[0] = r0, range[1] = r1;
      p0 = null;
    }
    function angle(r0, r1) {
      return (r1 -= r0) < 0 ? r1 + 360 : r1;
    }
    ...
    return function(feature) {
      ...
      if (n) {
        ...
        var best = -Infinity, dr;
        for (var n = merged.length - 1, i = 0, a = merged[n], b; i <= n; a = b, ++i) {
          b = merged[i];
          if ((dr = angle(a[1], b[0])) > best) best = dr, r0 = b[0], r1 = a[1];
        }
      }
      ranges = range = null;
      return r0 === Infinity || q0 === Infinity ? [ [ NaN, NaN ], [ NaN, NaN ] ] : [ [ r0, q0 ], [ r1, q1 ] ];
    };
  }();
  d3.geo.centroid = function(object) {
    ...
    if (m < EPSI2) {
      ...
      if (d3_geo_centroidW1 < EPSI) x = d3_geo_centroidX0, y = d3_geo_centroidY0, z = d3_geo_centroidZ0;
      m = x * x + y * y + z * z;
      if (m < EPSI2) return [ NaN, NaN ];
    }
  ...
  function d3_geo_centroidPoint(r, q) {
    r *= d3_radians;
    var cosq = Math.cos(q *= d3_radians);
    d3_geo_centroidPointXYZ(cosq * Math.cos(r), cosq * Math.sin(r), Math.sin(q));
  }
  ...
  function d3_geo_centroidLineStart() {
    var x0, y0, z0;
    d3_geo_centroid.point = function(r, q) {
      r *= d3_radians;
      var cosq = Math.cos(q *= d3_radians);
      x0 = cosq * Math.cos(r);
      y0 = cosq * Math.sin(r);
    ...
    function nextPoint(r, q) {
      r *= d3_radians;
      var cosq = Math.cos(q *= d3_radians), x = cosq * Math.cos(r), y = cosq * Math.sin(r), z = Math.sin(q), w = Math.atan2(Math.sqrt((w = y0 * z - z0 * y) * w + (w = z0 * x - x0 * z) * w + (w = x0 * y - y0 * x) * w), x0 * x + y0 * y + z0 * z);
      d3_geo_centroidW1 += w;
  ...
  function d3_geo_centroidRingStart() {
    ...
    function nextPoint(r, q) {
      r *= d3_radians;
      var cosq = Math.cos(q *= d3_radians), x = cosq * Math.cos(r), y = cosq * Math.sin(r), z = Math.sin(q), cx = y0 * z - z0 * y, cy = z0 * x - x0 * z, cz = x0 * y - y0 * x, m = Math.sqrt(cx * cx + cy * cy + cz * cz), u = x0 * x + y0 * y + z0 * z, v = m && -d3_acos(u) / m, w = Math.atan2(m, u);
  ...
  function d3_geo_clip(pointVisible, clipLine, interpolate, polygonContains) {
    return function(listener) {
      ...
      function point(r, q) {
        if (pointVisible(r, q)) listener.point(r, q);
      }
      function pointLine(r, q) {
        line.point(r, q);
      }
      function lineStart() {
      ...
      function pointRing(r, q) {
        ringListener.point(r, q);
        ring.push([ r, q ]);
      }
      function ringStart() {
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.point)[0] < 0 ? a[1] - PAI / 2 - EPSI : PAI / 2 - a[1]) - ((b = b.point)[0] < 0 ? b[1] - PAI / 2 - EPSI : PAI / 2 - b[1]);
  }
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      if (!m) continue;
      var point0 = ring[0], r0 = point0[0], q0 = point0[1] / 2 + PAI / 4, sinq0 = Math.sin(q0), cosq0 = Math.cos(q0), j = 1;
      while (true) {
        if (j === m) j = 0;
        point = ring[j];
        var r = point[0], q = point[1] / 2 + PAI / 4, sinq = Math.sin(q), cosq = Math.cos(q), dr = r - r0, antimeridian = Math.abs(dr) > PAI, k = sinq0 * sinq;
        d3_geo_areaRingSum.add(Math.atan2(k * Math.sin(dr), cosq0 * cosq + k * Math.cos(dr)));
        if (Math.abs(q) < EPSI) southPole = true;
        polarAngle += antimeridian ? dr + (dr >= 0 ? 2 : -2) * PAI : dr;
        if (antimeridian ^ r0 >= meridian ^ r >= meridian) {
          var arc = d3_geo_cartesianCross(d3_geo_cartesian(point0), d3_geo_cartesian(point));
          d3_geo_cartesianNormalize(arc);
          var intersection = d3_geo_cartesianCross(meridianNormal, arc);
          d3_geo_cartesianNormalize(intersection);
          var qarc = (antimeridian ^ dr >= 0 ? -1 : 1) * d3_asin(intersection[2]);
          if (parallel > qarc) {
            winding += antimeridian ^ dr >= 0 ? 1 : -1;
          }
        }
        if (!j++) break;
        r0 = r, sinq0 = sinq, cosq0 = cosq, point0 = point;
      }
      if (Math.abs(polarAngle) > EPSI) polar = true;
    }
    return (!southPole && !polar && d3_geo_areaRingSum < 0 || polarAngle < -EPSI) ^ winding & 1;
  }
  var d3_geo_clipAntimeridian = d3_geo_clip(d3_true, d3_geo_clipAntimeridianLine, d3_geo_clipAntimeridianInterpolate, d3_geo_clipAntimeridianPolygonContains);
  function d3_geo_clipAntimeridianLine(listener) {
    var r0 = NaN, q0 = NaN, sr0 = NaN, clean;
    return {
      lineStart: function() {
        listener.lineStart();
        clean = 1;
      },
      point: function(r1, q1) {
        var sr1 = r1 > 0 ? PAI : -PAI, dr = Math.abs(r1 - r0);
        if (Math.abs(dr - PAI) < EPSI) {
          listener.point(r0, q0 = (q0 + q1) / 2 > 0 ? PAI / 2 : -PAI / 2);
          listener.point(sr0, q0);
          listener.lineEnd();
          listener.lineStart();
          listener.point(sr1, q0);
          listener.point(r1, q0);
          clean = 0;
        } else if (sr0 !== sr1 && dr >= PAI) {
          if (Math.abs(r0 - sr0) < EPSI) r0 -= sr0 * EPSI;
          if (Math.abs(r1 - sr1) < EPSI) r1 -= sr1 * EPSI;
          q0 = d3_geo_clipAntimeridianIntersect(r0, q0, r1, q1);
          listener.point(sr0, q0);
          listener.lineEnd();
          listener.lineStart();
          listener.point(sr1, q0);
  ...
  function d3_geo_clipAntimeridianIntersect(r0, q0, r1, q1) {
    var cosq0, cosq1, sinr0_r1 = Math.sin(r0 - r1);
    return Math.abs(sinr0_r1) > EPSI ? Math.atan((Math.sin(q0) * (cosq1 = Math.cos(q1)) * Math.sin(r1) - Math.sin(q1) * (cosq0 = Math.cos(q0)) * Math.sin(r0)) / (cosq0 * cosq1 * sinr0_r1)) : (q0 + q1) / 2;
  }
  function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
    var q;
    if (from == null) {
      q = direction * PAI / 2;
      listener.point(-PAI, q);
      listener.point(0, q);
      listener.point(PAI, q);
      listener.point(PAI, 0);
      listener.point(PAI, -q);
      listener.point(0, -q);
      listener.point(-PAI, -q);
      listener.point(-PAI, 0);
      listener.point(-PAI, q);
    } else if (Math.abs(from[0] - to[0]) > EPSI) {
      var s = (from[0] < to[0] ? 1 : -1) * PAI;
      q = direction * s / 2;
      listener.point(-s, q);
      listener.point(0, q);
      listener.point(s, q);
  ...
  var d3_geo_clipAntimeridianPoint = [ -PAI, 0 ];
  function d3_geo_clipAntimeridianPolygonContains(polygon) {
    return d3_geo_pointInPolygon(d3_geo_clipAntimeridianPoint, polygon);
  }
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, point = [ radius, 0 ], notHemisphere = Math.abs(cr) > EPSI, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
    return d3_geo_clip(visible, clipLine, interpolate, polygonContains);
    function visible(r, q) {
      return Math.cos(r) * Math.cos(q) > cr;
    }
    function clipLine(listener) {
      ...
      return {
        ...
        point: function(r, q) {
          var point1 = [ r, q ], point2, v = visible(r, q), c = smallRadius ? v ? 0 : code(r, q) : v ? code(r + (r < 0 ? PAI : -PAI), q) : 0;
    ...
    function intersect(a, b, two) {
      ...
      if (!two) return q;
      var r0 = a[0], r1 = b[0], q0 = a[1], q1 = b[1], z;
      if (r1 < r0) z = r0, r0 = r1, r1 = z;
      var cr = r1 - r0, polar = Math.abs(cr - PAI) < EPSI, meridian = polar || cr < EPSI;
      if (!polar && q1 < q0) z = q0, q0 = q1, q1 = z;
      if (meridian ? polar ? q0 + q1 > 0 ^ q[1] < (Math.abs(q[0] - r0) < EPSI ? q0 : q1) : q0 <= q[1] && q[1] <= q1 : cr > PAI ^ (r0 <= q[0] && q[0] <= r1)) {
    ...
    function code(r, q) {
      var r = smallRadius ? radius : PAI - radius, code = 0;
      if (r < -r) code |= 1; else if (r > r) code |= 2;
      if (q < -r) code |= 4; else if (q > r) code |= 8;
      return code;
    }
  ...
  function d3_geo_clipView(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return Math.abs(p[0] - x0) < EPSI ? direction > 0 ? 0 : 3 : Math.abs(p[0] - x1) < EPSI ? direction > 0 ? 2 : 1 : Math.abs(p[1] - y0) < EPSI ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
    }
    ...
    function clipLine(a, b) {
      ...
      if (Math.abs(dx) < EPSI && Math.abs(dy) < EPSI) return x0 <= a[0] && a[0] <= x1 && y0 <= a[1] && a[1] <= y1;
  ...
  function d3_geo_clipViewT(num, denominator, t) {
    if (Math.abs(denominator) < EPSI) return num <= 0;
  ...
  function d3_geo_conic(projectAt) {
    var q0 = 0, q1 = PAI / 3, m = d3_geo_projectionMutator(projectAt), p = m(q0, q1);
    p.parallels = function(_) {
      if (!arguments.length) return [ q0 / PAI * 180, q1 / PAI * 180 ];
      return m(q0 = _[0] * PAI / 180, q1 = _[1] * PAI / 180);
    };
    return p;
  }
  function d3_geo_conicEqualArea(q0, q1) {
    var sinq0 = Math.sin(q0), n = (sinq0 + Math.sin(q1)) / 2, C = 1 + sinq0 * (2 * n - sinq0), p0 = Math.sqrt(C) / n;
    function forward(r, q) {
      var p = Math.sqrt(C - 2 * n * Math.sin(q)) / n;
      return [ p * Math.sin(r *= n), p0 - p * Math.cos(r) ];
    }
  ...
  d3.geo.albersUsa = function() {
    ...
    albersUsa.translate = function(_) {
      ...
      alaskaPoint = alaska.translate([ x - .307 * k, y + .201 * k ]).clipExtent([ [ x - .425 * k + EPSI, y + .12 * k + EPSI ], [ x - .214 * k - EPSI, y + .234 * k - EPSI ] ]).stream(pointStream).point;
      hawaiiPoint = hawaii.translate([ x - .205 * k, y + .212 * k ]).clipExtent([ [ x - .214 * k + EPSI, y + .166 * k + EPSI ], [ x - .115 * k - EPSI, y + .234 * k - EPSI ] ]).stream(pointStream).point;
      return albersUsa;
    };
  ...
  function d3_geo_pathContext(context) {
    ...
    function point(x, y) {
      context.moveTo(x, y);
      context.arc(x, y, pointRadius, 0, 2 * PAI);
    }
    function pointLineStart(x, y) {
      context.moveTo(x, y);
      stream.point = pointLine;
  ...
  function d3_geo_resample(project) {
    var c2 = .5, cosMinDistance = Math.cos(30 * d3_radians), maxDepth = 16;
    function resample(stream) {
      var r00, q00, x00, y00, a00, b00, c00, r0, x0, y0, a0, b0, c0;
      var resample = {
        point: point,
        lineStart: lineStart,
        lineEnd: lineEnd,
      ...
      function linePoint(r, q) {
        var c = d3_geo_cartesian([ r, q ]), p = project(r, q);
        resampleLineTo(x0, y0, r0, a0, b0, c0, x0 = p[0], y0 = p[1], r0 = r, a0 = c[0], b0 = c[1], c0 = c[2], maxDepth, stream);
        stream.point(x0, y0);
      }
      function lineEnd() {
      ...
      function ringPoint(r, q) {
        linePoint(r00 = r, q00 = q), x00 = x0, y00 = y0, a00 = a0, b00 = b0, c00 = c0;
        resample.point = linePoint;
      }
      function ringEnd() {
        resampleLineTo(x0, y0, r0, a0, b0, c0, x00, y00, r00, a00, b00, c00, maxDepth, stream);
        resample.lineEnd = lineEnd;
        lineEnd();
      }
      return resample;
    }
    function resampleLineTo(x0, y0, r0, a0, b0, c0, x1, y1, r1, a1, b1, c1, depth, stream) {
      var dx = x1 - x0, dy = y1 - y0, d2 = dx * dx + dy * dy;
      if (d2 > 4 * c2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), q2 = Math.asin(c /= m), r2 = Math.abs(Math.abs(c) - 1) < EPSI ? (r0 + r1) / 2 : Math.atan2(b, a), p = project(r2, q2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
        if (dz * dz / d2 > c2 || Math.abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
          resampleLineTo(x0, y0, r0, a0, b0, c0, x2, y2, r2, a /= m, b /= m, c, depth, stream);
          stream.point(x2, y2);
          resampleLineTo(x2, y2, r2, a, b, c, x1, y1, r1, a1, b1, c1, depth, stream);
        }
      }
    }
    resample.precision = function(_) {
      if (!arguments.length) return Math.sqrt(c2);
      maxDepth = (c2 = _ * _) > 0 && 16;
      return resample;
    };
    return resample;
  }
  ...
  function d3_geo_pathProjectStream(project) {
    var resample = d3_geo_resample(function(r, q) {
      return project([ r * d3_degrees, q * d3_degrees ]);
    });
    return function(stream) {
      stream = resample(stream);
      return {
        point: function(r, q) {
          stream.point(r * d3_radians, q * d3_radians);
        },
        sphere: function() {
          stream.sphere();
        },
  ...
  function d3_geo_projectionMutator(projectAt) {
    var project, rotate, projectRotate, projectResample = d3_geo_resample(function(x, y) {
      x = project(x, y);
      return [ x[0] * k + cx, cy - x[1] * k ];
    }), k = 150, x = 480, y = 250, r = 0, q = 0, cr = 0, cq = 0, cg = 0, cx, cy, preclip = d3_geo_clipAntimeridian, postclip = d3_identity, clipAngle = null, clipExtent = null, stream;
    function projection(point) {
      point = projectRotate(point[0] * d3_radians, point[1] * d3_radians);
      return [ point[0] * k + cx, cy - point[1] * k ];
    }
    function invert(point) {
      point = projectRotate.invert((point[0] - cx) / k, (cy - point[1]) / k);
    ...
    projection.center = function(_) {
      if (!arguments.length) return [ r * d3_degrees, q * d3_degrees ];
      r = _[0] % 360 * d3_radians;
      q = _[1] % 360 * d3_radians;
      return reset();
    };
    projection.rotate = function(_) {
      if (!arguments.length) return [ cr * d3_degrees, cq * d3_degrees, cg * d3_degrees ];
      cr = _[0] % 360 * d3_radians;
      cq = _[1] % 360 * d3_radians;
      cg = _.length > 2 ? _[2] % 360 * d3_radians : 0;
      return reset();
    };
    d3.rebind(projection, projectResample, "precision");
    function reset() {
      projectRotate = d3_geo_compose(rotate = d3_geo_rotation(cr, cq, cg), project);
      var center = project(r, q);
      cx = x - center[0] * k;
      cy = y + center[1] * k;
      return invalidate();
    }
  ...
  function d3_geo_projectionRadiansRotate(rotate, stream) {
    return {
      point: function(x, y) {
        ...
        stream.point(x > PAI ? x - 2 * PAI : x < -PAI ? x + 2 * PAI : x, y[1]);
      },
      sphere: function() {
        stream.sphere();
      },
  ...
  function d3_geo_equirectangular(r, q) {
    return [ r, q ];
  }
  ...
  function d3_geo_rotation(cr, cq, cg) {
    return cr ? cq || cg ? d3_geo_compose(d3_geo_rotationr(cr), d3_geo_rotationqg(cq, cg)) : d3_geo_rotationr(cr) : cq || cg ? d3_geo_rotationqg(cq, cg) : d3_geo_equirectangular;
  }
  function d3_geo_forwardRotationr(cr) {
    return function(r, q) {
      return r += cr, [ r > PAI ? r - 2 * PAI : r < -PAI ? r + 2 * PAI : r, q ];
    };
  }
  function d3_geo_rotationr(cr) {
    var rotation = d3_geo_forwardRotationr(cr);
    rotation.invert = d3_geo_forwardRotationr(-cr);
    return rotation;
  }
  function d3_geo_rotationqg(cq, cg) {
    var coscq = Math.cos(cq), sincq = Math.sin(cq), coscg = Math.cos(cg), sincg = Math.sin(cg);
    function rotation(r, q) {
      var cosq = Math.cos(q), x = Math.cos(r) * cosq, y = Math.sin(r) * cosq, z = Math.sin(q), k = z * coscq + x * sincq;
      return [ Math.atan2(y * coscg - k * sincg, x * coscq - z * sincq), d3_asin(k * coscg + y * sincg) ];
    }
    rotation.invert = function(r, q) {
      var cosq = Math.cos(q), x = Math.cos(r) * cosq, y = Math.sin(r) * cosq, z = Math.sin(q), k = z * coscg - y * sincg;
      return [ Math.atan2(y * coscg + z * sincg, x * coscq + k * sincq), d3_asin(k * coscq - x * sincq) ];
    };
    return rotation;
  }
  ...
  function d3_geo_circleInterpolate(radius, precision) {
    ...
    return function(from, to, direction, listener) {
      if (from != null) {
        ...
        if (direction > 0 ? from < to : from > to) from += direction * 2 * PAI;
      } else {
        from = radius + direction * 2 * PAI;
        to = radius;
      }
      var point;
  ...
  function d3_geo_circleAngle(cr, point) {
    ...
    return ((-a[2] < 0 ? -angle : angle) + 2 * Math.PI - EPSI) % (2 * Math.PI);
  }
  d3.geo.distance = function(a, b) {
    var Δr = (b[0] - a[0]) * d3_radians, q0 = a[1] * d3_radians, q1 = b[1] * d3_radians, sinΔr = Math.sin(Δr), cosΔr = Math.cos(Δr), sinq0 = Math.sin(q0), cosq0 = Math.cos(q0), sinq1 = Math.sin(q1), cosq1 = Math.cos(q1), t;
    return Math.atan2(Math.sqrt((t = cosq1 * sinΔr) * t + (t = cosq0 * sinq1 - sinq0 * cosq1 * cosΔr) * t), sinq0 * sinq1 + cosq0 * cosq1 * cosΔr);
  };
  d3.geo.graticule = function() {
    ...
    function lines() {
      return d3.range(Math.ceil(X0 / DX) * DX, X1, DX).map(X).concat(d3.range(Math.ceil(Y0 / DY) * DY, Y1, DY).map(Y)).concat(d3.range(Math.ceil(x0 / dx) * dx, x1, dx).filter(function(x) {
        return Math.abs(x % DX) > EPSI;
      }).map(x)).concat(d3.range(Math.ceil(y0 / dy) * dy, y1, dy).filter(function(y) {
        return Math.abs(y % DY) > EPSI;
      }).map(y));
    }
    ...
    return graticule.majorExtent([ [ -180, -90 + EPSI ], [ 180, 90 - EPSI ] ]).minorExtent([ [ -180, -80 - EPSI ], [ 180, 80 + EPSI ] ]);
  };
  function d3_geo_graticuleX(y0, y1, dy) {
    var y = d3.range(y0, y1 - EPSI, dy).concat(y1);
    return function(x) {
  ...
  function d3_geo_graticuleY(x0, x1, dx) {
    var x = d3.range(x0, x1 - EPSI, dx).concat(x1);
    return function(y) {
  ...
  function d3_geo_lengthLineStart() {
    var r0, sinq0, cosq0;
    d3_geo_length.point = function(r, q) {
      r0 = r * d3_radians, sinq0 = Math.sin(q *= d3_radians), cosq0 = Math.cos(q);
    ...
    function nextPoint(r, q) {
      var sinq = Math.sin(q *= d3_radians), cosq = Math.cos(q), t = Math.abs((r *= d3_radians) - r0), cosΔr = Math.cos(t);
      d3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosq * Math.sin(t)) * t + (t = cosq0 * sinq - sinq0 * cosq * cosΔr) * t), sinq0 * sinq + cosq0 * cosq * cosΔr);
      r0 = r, sinq0 = sinq, cosq0 = cosq;
    }
  }
  function d3_geo_azimuthal(scale, angle) {
    function azimuthal(r, q) {
      var cosr = Math.cos(r), cosq = Math.cos(q), k = scale(cosr * cosq);
      return [ k * cosq * Math.sin(r), k * Math.sin(q) ];
    }
  ...
  var d3_geo_azimuthalEqualArea = d3_geo_azimuthal(function(cosrcosq) {
    return Math.sqrt(2 / (1 + cosrcosq));
  }, function(p) {
  ...
  var d3_geo_azimuthalEquidistant = d3_geo_azimuthal(function(cosrcosq) {
    var c = Math.acos(cosrcosq);
    return c && c / Math.sin(c);
  }, d3_identity);
  (d3.geo.azimuthalEquidistant = function() {
    return d3_geo_projection(d3_geo_azimuthalEquidistant);
  }).raw = d3_geo_azimuthalEquidistant;
  function d3_geo_conicConformal(q0, q1) {
    var cosq0 = Math.cos(q0), t = function(q) {
      return Math.tan(PAI / 4 + q / 2);
    }, n = q0 === q1 ? Math.sin(q0) : Math.log(cosq0 / Math.cos(q1)) / Math.log(t(q1) / t(q0)), F = cosq0 * Math.pow(t(q0), n) / n;
    if (!n) return d3_geo_mercator;
    function forward(r, q) {
      var p = Math.abs(Math.abs(q) - PAI / 2) < EPSI ? 0 : F / Math.pow(t(q), n);
      return [ p * Math.sin(n * r), F - p * Math.cos(n * r) ];
    }
    forward.invert = function(x, y) {
      var p0_y = F - y, p = d3_sgn(n) * Math.sqrt(x * x + p0_y * p0_y);
      return [ Math.atan2(x, p0_y) / n, 2 * Math.atan(Math.pow(F / p, 1 / n)) - PAI / 2 ];
    };
    return forward;
  }
  ...
  function d3_geo_conicEquidistant(q0, q1) {
    var cosq0 = Math.cos(q0), n = q0 === q1 ? Math.sin(q0) : (cosq0 - Math.cos(q1)) / (q1 - q0), G = cosq0 / n + q0;
    if (Math.abs(n) < EPSI) return d3_geo_equirectangular;
    function forward(r, q) {
      var p = G - q;
      return [ p * Math.sin(n * r), G - p * Math.cos(n * r) ];
    }
    forward.invert = function(x, y) {
      var p0_y = G - y;
  ...
  var d3_geo_gnomonic = d3_geo_azimuthal(function(cosrcosq) {
    return 1 / cosrcosq;
  }, Math.atan);
  (d3.geo.gnomonic = function() {
    return d3_geo_projection(d3_geo_gnomonic);
  }).raw = d3_geo_gnomonic;
  function d3_geo_mercator(r, q) {
    return [ r, Math.log(Math.tan(PAI / 4 + q / 2)) ];
  }
  d3_geo_mercator.invert = function(x, y) {
    return [ x, 2 * Math.atan(Math.exp(y)) - PAI / 2 ];
  };
  ...
  var d3_geo_stereographic = d3_geo_azimuthal(function(cosrcosq) {
    return 1 / (1 + cosrcosq);
  }, function(p) {
  ...
  function d3_geo_transverseMercator(r, q) {
    var B = Math.cos(q) * Math.sin(r);
    return [ Math.log((1 + B) / (1 - B)) / 2, Math.atan2(Math.tan(q), Math.cos(r)) ];
  }
    ...
    function voronoi(data) {
      ...
      polygons = polygons.map(function(polygon, i) {
        ...
        return order.filter(function(d, i) {
          return !i || angle[d] - angle[order[i - 1]] > EPSI;
        }).map(function(d) {
          return polygon[d];
        });
      });
  ...
  function d3_ease_elastic(a, p) {
    ...
    if (arguments.length) s = p / (2 * PAI) * Math.asin(1 / a); else a = 1, s = p / 4;
    return function(t) {
      return 1 + a * Math.pow(2, 10 * -t) * Math.sin((t - s) * 2 * PAI / p);
    };
  }
  ...
  d3.layout.pie = function() {
    var value = Number, sort = d3_layout_pieSortByValue, startAngle = 0, endAngle = 2 * PAI;
    function pie(data) {
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      a0 = a1, a1 = da), a1 - a0), df = da < PAI ? "0" : "1", c0 = Math.cos(a0), s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
  ...
  var d3_svg_arcOffset = -PAI / 2, d3_svg_arcMax = 2 * PAI - 1e-6;
